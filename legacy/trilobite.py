...
MEASURE_BEVEL_DEG = None # 10 Sep 2026: when set (instrument2.measurement_bevel), add_hinge cuts the ventral bevel at THIS angle
                         # instead of P["maxAngle"]. The ruler no longer passes through schema.coerce's maxAngle clamp (hi = 40),
                         # which silently built a 40 deg bevel for every "bound 45" run.
...
def add_hinge(part, envelope, P, y_axis, rear, wide=False):
    ...
    part += blk
    phi, L = (P["maxAngle"] if MEASURE_BEVEL_DEG is None else float(MEASURE_BEVEL_DEG)) / 2, 80
